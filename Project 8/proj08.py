...
#######################
#   Computer Project #8
#   translate text into 7 different languages - Arabic, Spanish, Hindi, Russian,\
#      French, German, and Italian
#   scans text so be able to translate all words ignoring capital letters as \
#   well as punctuation. only considers the periods when attached to a word. 
#   first translates the text using a list
#   then translates the text using a dictionary
#   displays the amount of time that it took to complete each translation.
#
#
#######################
...
def getDictFormattedDatabases(listDatabases):
    ...
    """
    L = []
    for langu_name, langu_db in listDatabases:
        L.append((langu_name, dict(langu_db))) 
    return L
    #Language DB is in a dictionary when appending it to the list
    #converts from list to dictionary
"""
Translates text from English to the language of the specified DB. The specified
...
def translateFromDict(text, dictBasedDb):
    ...
    for word in words:
        ...
        if word [-1]== '.':
            word = word[:-1]
            toAppend = '.'
        if word in dictBasedDb: #check for word to be in the dictionary db
            transword = dictBasedDb[word]
        else:   #if its not in dictionary db, leave the word as it is.
            transword = word
        translate += transword + toAppend + ' '
...
def performDictBasedTranslations(preprocessedText, dictBasedDatabases):
    ...
    """
    for tup in dictBasedDatabases: #the tup consist of the 7 different languages
        translatedText = translateFromDict(preprocessedText, tup [1])
        display (tup[0], translatedText)
        #display the langauage name followed by the text that was translated
"""
*******************************************************************************
